...
public class ReportController {
    ...
    private final ExpenseService expenseService;  // Added ExpenseService
    ...
    public ReportController(
            ...
            ExpenseService expenseService) {  // Added ExpenseService parameter
        this.csvExportService = csvExportService;
        this.pdfExportService = pdfExportService;
        this.emailService = emailService;
        this.expenseService = expenseService;  // Initialize ExpenseService
    }
    ...
    public void exportExpenses(HttpServletResponse response) throws IOException {
        List<Expense> expenses = expenseService.getAllExpenses();  // Use the instance
    ...
    public String sendExpenseReportEmail() throws Exception {
        List<Expense> expenses = expenseService.getAllExpenses();  // Use the instance
    ...
    private ByteArrayResource generatePdfReportAsResource(List<Expense> expenses) throws Exception {
        ...
        com.itextpdf.layout.element.Table table = new com.itextpdf.layout.element.Table(
                com.itextpdf.layout.properties.UnitValue.createPercentArray(new float[]{1, 2, 2, 2, 3}))
                .useAllAvailableWidth();

        // Add headers
        table.addHeaderCell(new com.itextpdf.layout.element.Cell().add(new com.itextpdf.layout.element.Paragraph("ID")));
        table.addHeaderCell(new com.itextpdf.layout.element.Cell().add(new com.itextpdf.layout.element.Paragraph("Category")));
        ...
        for (Expense expense : expenses) {
            table.addCell(new com.itextpdf.layout.element.Cell().add(new com.itextpdf.layout.element.Paragraph(String.valueOf(expense.getId()))));
            table.addCell(new com.itextpdf.layout.element.Cell().add(new com.itextpdf.layout.element.Paragraph(expense.getCategory())));
            table.addCell(new com.itextpdf.layout.element.Cell().add(new com.itextpdf.layout.element.Paragraph(String.valueOf(expense.getAmount()))));
            table.addCell(new com.itextpdf.layout.element.Cell().add(new com.itextpdf.layout.element.Paragraph(expense.getDate().toString())));
            table.addCell(new com.itextpdf.layout.element.Cell().add(new com.itextpdf.layout.element.Paragraph(expense.getDescription())));
        }
